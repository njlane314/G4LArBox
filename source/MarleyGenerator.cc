...
namespace G4LArBox {
    MarleyGenerator::MarleyGenerator(std::string marley_json) 
    {
        ...
        {
            std::cout << "MARLEY configuration file not found! Continuing..." << std::endl;
        }
    }
    ...
    void MarleyGenerator::GeneratePrimaryVertex(G4Event* event) 
    {
        ...
        for (const auto& marley_particle : marley_event.get_final_particles()) 
        {
            ...
            if (particle_idx > 1 && marley_residue_pdg == 1000190400 && cascade_idx < marley_event.get_cascade_levels().size()) 
            {
                ...
                {
                    double time_of_decay = SampleFiniteExcitedTime(iter->second);
    ...
    double MarleyGenerator::SampleFinitExcitedTime(double half_life) 
    {
